...
public class AdhesionResource {
     ...
     */
    @RequestMapping(value = "/adhesions",
     ...
     */
    @RequestMapping(value = "/adhesions/{id}",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Adhesion> get(@PathVariable Long id) {
        log.debug("REST request to get Adhesion : {}", id);
        return Optional.ofNullable(adhesionRepository.findOne(id))
            .map(adhesion -> new ResponseEntity<>(
                adhesion,
                HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }
